...
function SystemStatus() {
  ...
  return (
    <div style={{
      ...
      <div suppressHydrationWarning style={{ marginTop: 10, paddingTop: 8, borderTop: '1px solid rgba(0,255,136,0.1)', fontFamily: 'var(--font-mono)', fontSize: 8, color: 'var(--muted)', letterSpacing: 1 }}>
...
function FooterSynthwave({ siteConfig, sectionLinks, platformLinks, socialLinks, hasAdminAccess, handleHashLink, year }) {
  return (
    <footer style={{ borderTop: '2px solid var(--green)', background: 'var(--bg)', position: 'relative', zIndex: 1, overflow: 'hidden' }}>
      ...
      <div aria-hidden style={{ position: 'absolute', inset: 0, backgroundImage: 'linear-gradient(rgba(0,255,136,0.04) 1px,transparent 1px),linear-gradient(90deg,rgba(0,212,255,0.04) 1px,transparent 1px)', backgroundSize: '28px 28px', pointerEvents: 'none' }} />
      <div aria-hidden style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 80, background: 'linear-gradient(transparent, rgba(0,255,136,0.05))', pointerEvents: 'none' }} />
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                {socialLinks.map(({ href, icon, label }) => (
                  <a key={label} href={href} target="_blank" rel="noopener noreferrer" title={label}
                    ...
                    onMouseEnter={e => { e.currentTarget.style.borderColor='var(--green)'; e.currentTarget.style.color='var(--green)'; e.currentTarget.style.boxShadow='0 0 10px rgba(0,255,136,0.2)' }}
                    onMouseLeave={e => { e.currentTarget.style.borderColor='var(--border)'; e.currentTarget.style.color='var(--muted)'; e.currentTarget.style.boxShadow='none' }}
                  >{icon}</a>
                ))}
              </div>
...
export default function Footer() {
  ...
  const FS = {
    ...
    dock:         { bg: '#061018',    text: '#c8d8e8',      accent: 'var(--cyan)',  sub: 'var(--green)', muted: '#6b8296',     border: 'rgba(0,212,255,0.18)', topBorder: '1px solid rgba(0,212,255,0.26)' },
  ...
  return (
    <footer suppressHydrationWarning style={{ borderTop: fs.topBorder, background: fs.bg, position: 'relative', zIndex: 1, overflow: 'hidden' }}>
      ...
      <style>{`
        ...
        .ft-social:hover { border-color: var(--green) !important; color: var(--green) !important; background: rgba(0,255,136,0.06) !important; }
      `}</style>
