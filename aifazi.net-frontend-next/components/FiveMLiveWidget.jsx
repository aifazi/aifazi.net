  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, maxWidth: 560 }}>
      <div style={{ background: 'color-mix(in srgb, var(--bg2) 92%, var(--bg) 8%)', border: `1px solid ${isUp ? 'color-mix(in srgb, var(--green) 18%, transparent)' : 'var(--border)'}`, borderRadius: 10, padding: '14px 16px' }}>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, letterSpacing: 2, color: 'var(--muted)', marginBottom: 6 }}>STATUS</div>
        ...
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--muted)', marginTop: 4 }}>{err ? String(err).slice(0, 40) : 'All systems operational'}</div>
      </div>
      <div style={{ background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 10, padding: '14px 16px' }}>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, letterSpacing: 2, color: 'var(--muted)', marginBottom: 6 }}>{playersOnline != null ? 'PLAYERS' : 'PING'}</div>
        ...
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--muted)', marginTop: 4 }}>{playersOnline != null ? (isUp ? 'online' : 'offline') : ping?.status || 'checking'}</div>
      </div>
      <div style={{ background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 10, padding: '14px 16px' }}>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, letterSpacing: 2, color: 'var(--muted)', marginBottom: 6 }}>UPTIME</div>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 14, fontWeight: 700, color: 'var(--cyan)' }}>{data?.uptime_human || data?.uptime || '—'}</div>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--muted)', marginTop: 4 }}>since last deploy</div>
      </div>
    </div>
  )
}
