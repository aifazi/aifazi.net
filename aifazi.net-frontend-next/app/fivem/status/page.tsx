  ...
  return (
    <div style={{ minHeight: '100vh', color: 'var(--text)', fontFamily: 'var(--font-mono)' }}>
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '60px 24px' }}>
        ...
        {/* Status badge */}
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginTop: 16, padding: '10px 20px', borderRadius: 10, background: online ? 'rgba(0,255,136,0.08)' : 'rgba(255,71,87,0.08)', border: `1px solid ${online ? 'rgba(0,255,136,0.3)' : 'rgba(255,71,87,0.3)'}` }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16, marginTop: 24 }}>
          <div style={{ padding: 20, borderRadius: 10, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)' }}>
            <div style={{ fontSize: 10, color: '#8b949e', letterSpacing: 2 }}>PLAYERS</div>
            <div style={{ fontSize: 28, fontWeight: 800, color: C, marginTop: 4 }}>{playerCount}/{status?.max_players || 128}</div>
          </div>
          <div style={{ padding: 20, borderRadius: 10, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)' }}>
            <div style={{ fontSize: 10, color: '#8b949e', letterSpacing: 2 }}>UPTIME</div>
            <div style={{ fontSize: 28, fontWeight: 800, color: G, marginTop: 4 }}>{status?.uptime_label || '—'}</div>
          </div>
          <div style={{ padding: 20, borderRadius: 10, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)' }}>
            <div style={{ fontSize: 10, color: '#8b949e', letterSpacing: 2 }}>LAST HEARTBEAT</div>
            <div style={{ fontSize: 14, color: '#c9d1d9', marginTop: 8 }}>{status?.last_seen_label || (status?.last_seen ? new Date(status.last_seen).toLocaleTimeString() : '—')}</div>
          </div>
        </div>
          ...
          <div style={{ marginTop: 32 }}>
            ...
            <div style={{ borderRadius: 10, border: '1px solid rgba(255,255,255,0.06)', overflow: 'hidden' }}>
              {players.map((p: any, i: number) => (
                <div key={i} style={{ padding: '8px 16px', background: i % 2 ? 'rgba(255,255,255,0.01)' : 'transparent', borderBottom: '1px solid rgba(255,255,255,0.03)', fontSize: 12, display: 'flex', justifyContent: 'space-between' }}>
                  <span>{p.name || p.username || `Player ${i+1}`}</span>
                  <span style={{ color: '#8b949e' }}>{p.ping ? `${p.ping}ms` : ''}</span>
                </div>
              ))}
            </div>
          </div>
