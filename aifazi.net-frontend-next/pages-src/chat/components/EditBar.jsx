...
export function EditBar({ msg, onSave, onCancel }) {
  ...
  return (
    <div style={{ padding: '8px 14px', background: 'rgba(255,215,0,0.06)', borderTop: `1px solid rgba(255,215,0,0.15)`, display: 'flex', gap: 8 }}>
      ...
      <button onClick={onCancel} style={{ padding: '6px 12px', border: `1px solid ${T.border}`, borderRadius: 8, background: 'transparent', color: T.muted, cursor: 'pointer', fontSize: 14 }}>âœ•</button>
    </div>
  )
}
...
export function ReplyBar({ replyTo, onCancel }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 14px', background: 'color-mix(in srgb, var(--cyan) 6%, transparent)', borderTop: `1px solid color-mix(in srgb, var(--cyan) 15%, transparent)`, flexShrink: 0 }}>
      ...
      <button onClick={onCancel} style={{ padding: '2px 7px', border: `1px solid ${T.border}`, borderRadius: 6, background: 'transparent', color: T.muted, cursor: 'pointer', fontSize: 12 }}>âœ•</button>
    </div>
  )
}
