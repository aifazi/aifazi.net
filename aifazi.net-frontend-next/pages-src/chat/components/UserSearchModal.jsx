          ...
          {results.map(u => (
            <div key={u.id || u.username} onClick={() => setSelectedUser(u)}
              style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 10px', borderRadius: 8, cursor: 'pointer',
                background: selectedUser?.username === u.username ? 'rgba(0,255,136,0.1)' : 'transparent',
                border: selectedUser?.username === u.username ? `1px solid rgba(0,255,136,0.25)` : '1px solid transparent', transition: 'all 0.15s' }}
              onMouseEnter={e => { if (selectedUser?.username !== u.username) e.currentTarget.style.background = 'rgba(255,255,255,0.04)' }}
              onMouseLeave={e => { e.currentTarget.style.background = selectedUser?.username === u.username ? 'rgba(0,255,136,0.1)' : 'transparent' }}>
        ...
        <div style={{ padding: '10px 16px 14px', borderTop: `1px solid ${T.border}`, display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
          ...
          <button onClick={handleSelect} disabled={!selectedUser}
            style={{ padding: '7px 16px', border: 'none', borderRadius: 8,
              background: selectedUser ? 'linear-gradient(135deg,rgba(0,255,136,0.85),rgba(0,212,255,0.85))' : 'rgba(255,255,255,0.06)',
