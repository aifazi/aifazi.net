  ...
  return (
    <div ref={elRef || elRef2} onScroll={onScroll} style={{ flex: 1, overflowY: 'auto', padding: '8px 0' }}>
      {multiSelect && selectedIds.length > 0 && (
        <div style={{ position:'sticky', top:0, zIndex:10, display:'flex', alignItems:'center', gap:8, padding:'6px 14px', background:'rgba(0,212,255,0.08)', borderBottom:`1px solid rgba(0,212,255,0.2)`, marginBottom:4, fontFamily:T.mono, fontSize:10 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              {group.map((m, gi) => (
                <div key={m.id || gi} style={{ display:'flex', gap:8, marginBottom:2, alignItems:'flex-start',
                  background: selectedIds.includes(m.id) ? 'rgba(0,255,136,0.06)' : activeMsg === m.id ? 'rgba(0,212,255,0.04)' : 'transparent' }}>
                  {multiSelect && (
