...
const S = {
  ...
  success:  { color:'var(--green)', fontFamily:'var(--font-mono)', fontSize:11, marginTop:8, padding:'8px 12px', background:'rgba(0,255,136,.07)', border:'1px solid rgba(0,255,136,.25)' },
...
function DropZone({ onFiles, accept='*', multiple=false, label='Drop files here or click to browse' }) {
  ...
  return (
    <div
      ...
      style={{ border:`2px dashed ${over ? 'var(--green)' : 'var(--border)'}`, padding:'40px 24px', textAlign:'center', cursor:'pointer', transition:'all .2s', background: over ? 'rgba(0,255,136,.04)' : 'var(--bg3)' }}
    >
...
export { loadScript, loadMammoth, loadTesseract, downloadBlob, readText, readDataUrl, fmtBytes, DropZone, ToolHeader, FileBadge, Progress }
