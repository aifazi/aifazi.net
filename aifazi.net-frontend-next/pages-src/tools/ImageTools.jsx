...
import { loadTesseract, downloadBlob, readDataUrl, fmtBytes, DropZone, ToolHeader, FileBadge, Progress } from './sharedTools'
    ...
    <div>
      ...
      {orig && (
        <div style={{ marginTop:16 }}>
          ...
          <button onClick={resize} style={{ ...S.btn, marginTop:16 }}>⬇️ RESIZE & DOWNLOAD</button>
        ...
        <div style={{ marginTop:16 }}>
          ...
          <div style={{ display:'flex', gap:12, marginTop:16, alignItems:'center', flexWrap:'wrap' }}>
            ...
            <button onClick={convert} disabled={busy} style={{ ...S.btn, alignSelf:'end' }}>
              {busy ? `CONVERTING ${done}/${files.length}...` : `⬇️ CONVERT ${files.length} IMAGE(S)`}
            </button>
          </div>
      ...
      {orig && (
        <div style={{ marginTop:16 }}>
          <div style={{ display:'flex', gap:8, marginBottom:16 }}>
            {[['horizontal','[H] Flip Horizontal'],['vertical','[V] Flip Vertical']].map(([v,l]) => (
          ...
          <div style={{ display:'flex', gap:8 }}>
            ...
            {result && <button onClick={() => downloadBlob(result.blob, orig.file.name.replace(/\.[^.]+$/, '_flipped.jpg'))} style={S.btnOut}>⬇️ DOWNLOAD</button>}
          </div>
        </div>
      )}
    </div>
      ...
      {base && (
        <div style={{ marginTop:16 }}>
          ...
          <div style={{ display:'flex', gap:8 }}>
            ...
            {result && <button onClick={()=>downloadBlob(result.blob, base.file.name.replace(/\.[^.]+$/,'_wm.jpg'))} style={S.btnOut}>⬇️ DOWNLOAD</button>}
          </div>
        </div>
      )}
    </div>
    ...
    <div>
      ...
      {file && (
        <div style={{ marginTop:16 }}>
          ...
          <button onClick={convert} disabled={busy} style={{ ...S.btn, marginTop:12 }}>{busy ? `EXPORTING... ${pct}%` : '⬇️ EXPORT ALL AS JPG'}</button>
        </div>
      )}
...
export { CompressImage, ResizeImage, ConvertImage, ImageOCR, FlipImage, ImageWatermark, JPGToPDF, PDFToJPG }
