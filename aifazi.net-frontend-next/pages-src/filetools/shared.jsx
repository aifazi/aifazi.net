...
const S = {
  ...
  btnSm:    { fontFamily:'var(--font-mono)', fontSize: 11, letterSpacing:2, padding:'7px 14px', background:'var(--green)', color:'#000', border:'none', cursor:'pointer', fontWeight:700 },
...
function ToolHeader({ tool, onBack }) {
  return (
    <div style={{ display:'flex', alignItems:'center', gap:16, marginBottom:28 }}>
      <button onClick={onBack} style={{ ...S.btnOut, padding:'8px 14px', fontSize: 11 }}>← BACK</button>
      ...
      <div>
        ...
        <div style={{ fontFamily:'var(--font-mono)', fontSize: 11, color:'var(--muted)', letterSpacing:2, marginTop:2 }}>{tool.desc}</div>
      </div>
    </div>
  )
}
...
function FileBadge({ file, onRemove }) {
  return (
    <div style={{ display:'flex', alignItems:'center', gap:10, background:'var(--bg3)', border:'1px solid var(--border)', padding:'8px 14px', marginBottom:6 }}>
      ...
      <span style={{ fontFamily:'var(--font-mono)', fontSize: 11, color:'var(--muted)' }}>{fmtBytes(file.size)}</span>
      {onRemove && <button onClick={onRemove} style={{ background:'none', border:'none', color:'var(--red)', cursor:'pointer', fontSize:14, padding:'0 4px' }} aria-label="Close">✕</button>}
    </div>
  )
}
...
function Progress({ pct, label }) {
  return (
    <div style={{ marginTop:16 }}>
      <div style={{ fontFamily:'var(--font-mono)', fontSize: 11, color:'var(--muted)', marginBottom:6, letterSpacing:2 }}>{label || 'PROCESSING...'} {pct}%</div>
