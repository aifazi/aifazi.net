  ...
  const runOp = async () => {
    ...
    try {
      if (op === 'transfer') {
        ...
        toast.success(`${op.toUpperCase()} ${qty} → new on-hand ${r.data?.new_stock ?? r.data?.after}`, { title: 'Inventory' })
      }
      setNote(''); load()
    } catch (e) {
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(280px,380px) 1fr', gap: 16, alignItems: 'start', marginBottom: 18 }}>
        ...
        <div>
          <div style={{ display: 'flex', gap: 8, marginBottom: 10, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            <button onClick={() => setShowLocs(v => !v)} style={{ fontFamily: MONO, fontSize: 9, letterSpacing: 1, padding: '8px 12px', background: 'rgba(0,212,255,.1)', border: '1px solid rgba(0,212,255,.4)', color: C, borderRadius: 6, cursor: 'pointer' }}>{showLocs ? 'HIDE LOCATIONS' : 'LOCATIONS'}</button>
          </div>
        ...
        {movements.length === 0 ? <div style={{ fontFamily: MONO, fontSize: 10, color: 'var(--muted)' }}>No movements yet.</div> : (
          movements.map(m => (
            <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '6px 0', borderBottom: '1px solid var(--border)', flexWrap: 'wrap' }}>
              ...
              <span style={{ fontFamily: MONO, fontSize: 9, letterSpacing: 1, padding: '1px 6px', borderRadius: 10, background: 'rgba(0,212,255,.08)', border: '1px solid rgba(0,212,255,.3)', color: C }}>{m.reason}</span>
