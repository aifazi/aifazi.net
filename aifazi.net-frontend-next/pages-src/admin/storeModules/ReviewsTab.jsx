      ...
      {tab === 'reviews' && (
        <>
          <div style={{ display: 'flex', gap: 8, marginBottom: 10, flexWrap: 'wrap', alignItems: 'center' }}>
            <div style={{ fontFamily: MONO, fontSize: 11, letterSpacing: 2, color: 'var(--muted)' }}>MODERATION QUEUE</div>
            <div style={{ flex: 1 }} />
            {[['all', `ALL (${reviews.length})`], ['pending', `PENDING (${counts.pending || 0})`], ['approved', `APPROVED (${counts.approved || 0})`], ['rejected', `REJECTED (${counts.rejected || 0})`]].map(([v, l]) => (
              <button key={v} onClick={() => setFilter(v)} style={{ fontFamily: MONO, fontSize: 11, letterSpacing: 1, padding: '5px 10px', borderRadius: 20, cursor: 'pointer', background: filter === v ? 'color-mix(in srgb, var(--green) 12%, transparent)' : 'transparent', color: filter === v ? G : 'var(--muted)', border: `1px solid ${filter === v ? 'color-mix(in srgb, var(--green) 40%, transparent)' : 'var(--border)'}` }}>{l}</button>
            ))}
          </div>
              ...
              return (
                <div key={r.id} style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 10, padding: '12px 14px', marginBottom: 8 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                    <span style={{ fontFamily: MONO, fontSize: 11, letterSpacing: 1, padding: '2px 8px', borderRadius: 12, background: `${col}14`, border: `1px solid ${col}44`, color: col }}>{r.status.toUpperCase()}</span>
                    <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--text)' }}>{r.username || '—'}</span>
                    <Stars n={r.rating} />
                    <span style={{ fontFamily: MONO, fontSize: 11, color: 'var(--muted)' }}>{r.product_name || r.product_id}</span>
                    <div style={{ flex: 1 }} />
                    <span style={{ fontFamily: MONO, fontSize: 11, color: 'var(--muted)' }}>{fmt(r.created_at)}</span>
                    <div style={{ display: 'flex', gap: 6 }}>
                      {r.status !== 'approved' && <button onClick={() => setStatus(r, 'approved')} style={{ fontFamily: MONO, fontSize: 11, padding: '5px 10px', background: 'color-mix(in srgb, var(--green) 10%, transparent)', border: '1px solid color-mix(in srgb, var(--green) 40%, transparent)', color: G, borderRadius: 6, cursor: 'pointer' }}>✓ APPROVE</button>}
                      {r.status !== 'rejected' && <button onClick={() => setStatus(r, 'rejected')} style={{ fontFamily: MONO, fontSize: 11, padding: '5px 10px', background: 'rgba(255,71,87,.1)', border: '1px solid rgba(255,71,87,.4)', color: R, borderRadius: 6, cursor: 'pointer' }}>✕ REJECT</button>}
                      <button onClick={() => deleteReview(r)} style={{ fontFamily: MONO, fontSize: 11, padding: '5px 10px', background: 'transparent', border: '1px solid var(--border)', color: 'var(--muted)', borderRadius: 6, cursor: 'pointer' }}>DEL</button>
                    </div>
                  </div>
      ...
      {tab === 'testimonials' && (
        <>
          <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 12, padding: 16, marginBottom: 16 }}>
            <div style={{ fontFamily: MONO, fontSize: 11, letterSpacing: 2, color: 'var(--muted)', marginBottom: 12 }}>{editingT ? 'EDIT TESTIMONIAL' : 'NEW TESTIMONIAL'}</div>
            ...
            testimonials.map(t => (
              <div key={t.id} style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 10, padding: '12px 14px', marginBottom: 8 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                  ...
                  {t.role && <span style={{ fontFamily: MONO, fontSize: 11, color: 'var(--muted)' }}>{t.role}</span>}
                  <span style={{ fontFamily: MONO, fontSize: 11, letterSpacing: 1, padding: '2px 8px', borderRadius: 12, background: `${STAT_COLORS[t.status] || 'var(--muted)'}14`, border: `1px solid ${STAT_COLORS[t.status] || 'var(--muted)'}44`, color: STAT_COLORS[t.status] || 'var(--muted)' }}>{(t.status || 'pending').toUpperCase()}</span>
                  <div style={{ flex: 1 }} />
                  <span style={{ fontFamily: MONO, fontSize: 11, color: 'var(--muted)' }}>order {t.display_order || 0}</span>
                  <button onClick={() => editTestimonial(t)} style={{ fontFamily: MONO, fontSize: 11, padding: '5px 10px', background: 'color-mix(in srgb, var(--cyan) 10%, transparent)', border: '1px solid color-mix(in srgb, var(--cyan) 40%, transparent)', color: C, borderRadius: 6, cursor: 'pointer' }}>EDIT</button>
                  <button onClick={() => deleteTestimonial(t)} style={{ fontFamily: MONO, fontSize: 11, padding: '5px 10px', background: 'transparent', border: '1px solid var(--border)', color: 'var(--muted)', borderRadius: 6, cursor: 'pointer' }}>DEL</button>
                </div>
