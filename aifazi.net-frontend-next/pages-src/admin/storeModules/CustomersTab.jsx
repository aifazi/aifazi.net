    ...
    <div>
      <div style={{ display: 'flex', gap: 10, marginBottom: 16, alignItems: 'center', flexWrap: 'wrap' }}>
        <div style={{ flex: '1 1 260px', position: 'relative' }}>
          ...
          <span style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', fontSize: 12, opacity: 0.5 }}>ðŸ”</span>
        </div>
        <button onClick={load} style={{ fontFamily: MONO, fontSize: 9, letterSpacing: 1, padding: '8px 14px', background: 'var(--bg3)', border: '1px solid var(--border)', color: 'var(--muted)', cursor: 'pointer', borderRadius: 6 }}>â†» REFRESH</button>
          ...
          <div key={c.id} style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 10, marginBottom: 8, overflow: 'hidden' }}>
            <div onClick={() => openId === c.id ? setOpenId(null) : openDetail(c)} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: 12, cursor: 'pointer', flexWrap: 'wrap' }}>
              ...
              <div style={{ flex: 1, minWidth: 140 }}>
                ...
                <div style={{ fontFamily: MONO, fontSize: 10, color: 'var(--muted)', marginTop: 2 }}>{c.email || 'no email'} Â· joined {fmt(c.created_at)}</div>
              </div>
              <Stat label="ORDERS" value={c.orders_count} color="var(--text)" />
              <Stat label="SPENT" value={money(c.spent_cents)} color={G} />
              <span style={{ fontFamily: MONO, fontSize: 16, color: 'var(--muted)' }}>{openId === c.id ? 'â–¾' : 'â–¸'}</span>
            </div>
              ...
              <div style={{ padding: '0 14px 14px', borderTop: '1px solid var(--border)' }}>
                  ...
                  <div style={{ marginBottom: 14 }}>
                    ...
                    {detail.orders.map(o => (
                      <div key={o.id} style={{ background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 8, padding: '10px 12px', marginBottom: 6, display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                        ...
                        {o.coupon_code && <span style={{ fontFamily: MONO, fontSize: 9, color: Y }}>ðŸŽŸ {o.coupon_code}</span>}
                        <div style={{ flex: 1 }} />
                ...
                <div style={{ marginBottom: 14 }}>
                  ...
                  <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
                    <input value={note} onChange={e => setNote(e.target.value)} placeholder="Add an internal noteâ€¦" style={{ ...INP, flex: 1 }} />
                  ...
                  {detail.notes?.length > 0 ? detail.notes.map(n => (
                    <div key={n.id} style={{ background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 8, padding: '9px 12px', marginBottom: 6, display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontFamily: MONO, fontSize: 9, color: 'var(--muted)', marginTop: 3 }}>{n.staff_name || 'staff'} Â· {fmt(n.created_at)}</div>
                      </div>
                      <button onClick={() => delNote(c.id, n.id)} style={{ background: 'none', border: 'none', color: R, cursor: 'pointer', fontSize: 13 }}>âœ•</button>
                    </div>
