  ...
  return (
    <div>
          ...
          <div style={{ display: 'grid', gap: 6 }}>
            {low.map(i => (
              <div key={i.kind + i.id} style={{ background: 'var(--bg3)', border: '1px solid rgba(250,204,21,.25)', borderRadius: 8, padding: '9px 12px', display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                <span style={{ fontFamily: MONO, fontSize: 11, letterSpacing: 1, padding: '2px 8px', borderRadius: 12, background: 'rgba(250,204,21,.1)', border: '1px solid rgba(250,204,21,.3)', color: Y }}>{i.kind.toUpperCase()}</span>
                <div style={{ flex: 1, minWidth: 140 }}>
                  <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)' }}>{i.name}</div>
                  <div style={{ fontFamily: MONO, fontSize: 11, color: 'var(--muted)', marginTop: 2 }}>{i.sku || 'no sku'}</div>
                </div>
                <span style={{ fontFamily: MONO, fontSize: 13, fontWeight: 700, color: R }}>{i.stock_qty ?? 0} left</span>
                <span style={{ fontFamily: MONO, fontSize: 11, color: 'var(--muted)' }}>threshold {i.low_stock_threshold ?? 0}</span>
              </div>
            ))}
          </div>
        )}
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 10, flexWrap: 'wrap', alignItems: 'center' }}>
        <div style={{ fontFamily: MONO, fontSize: 11, letterSpacing: 2, color: 'var(--muted)' }}>STOCK LEDGER</div>
        <div style={{ flex: 1 }} />
        {['all', ...reasons].map(rsn => (
          <button key={rsn} onClick={() => setFilter(rsn)} style={{ fontFamily: MONO, fontSize: 11, letterSpacing: 1, padding: '5px 10px', borderRadius: 20, cursor: 'pointer', background: filter === rsn ? 'color-mix(in srgb, var(--green) 12%, transparent)' : 'transparent', color: filter === rsn ? G : 'var(--muted)', border: `1px solid ${filter === rsn ? 'color-mix(in srgb, var(--green) 40%, transparent)' : 'var(--border)'}` }}>{rsn.toUpperCase()}</button>
        ))}
      </div>
        ...
        shown.map(l => {
          ...
          return (
            <div key={l.id} style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 8, padding: '9px 13px', marginBottom: 5, display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
              ...
              <span style={{ fontFamily: MONO, fontSize: 11, letterSpacing: 1, padding: '2px 8px', borderRadius: 12, background: `${col}14`, border: `1px solid ${col}44`, color: col, minWidth: 80, textAlign: 'center' }}>{(l.reason || 'adjustment').toUpperCase()}</span>
              <div style={{ flex: 1, minWidth: 140 }}>
                <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text)' }}>{l.product_name || l.product_id || '—'}</div>
                <div style={{ fontFamily: MONO, fontSize: 11, color: 'var(--muted)', marginTop: 2 }}>{l.note || (l.ref_type === 'variant' ? 'variant update' : 'stock movement')} · {fmt(l.created_at)}</div>
              </div>
              <span style={{ fontFamily: MONO, fontSize: 11, color: 'var(--muted)' }}>by {l.actor || 'system'}</span>
            </div>
          )
        })
      )}
