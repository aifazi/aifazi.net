        ...
        deals.map(d => {
          ...
          return (
            <div key={d.id} style={{ background: 'var(--bg2)', border: `1px solid ${active ? 'var(--border)' : 'rgba(255,255,255,.08)'}`, opacity: active ? 1 : 0.55, borderRadius: 10, padding: '12px 14px', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
              ...
              <span style={{ fontFamily: MONO, fontSize: 9, letterSpacing: 1, padding: '2px 8px', borderRadius: 12, background: active ? (live ? 'rgba(0,255,136,.08)' : 'rgba(0,212,255,.08)') : 'rgba(255,71,87,.08)', border: `1px solid ${active ? (live ? 'rgba(0,255,136,.3)' : 'rgba(0,212,255,.3)') : 'rgba(255,71,87,.3)'}`, color: active ? (live ? G : C) : R }}>{active ? (live ? '● LIVE' : 'SCHEDULED') : 'PAUSED'}</span>
              ...
              <button onClick={() => startEdit(d)} style={{ fontFamily: MONO, fontSize: 9, padding: '6px 10px', background: 'rgba(0,212,255,.1)', border: '1px solid rgba(0,212,255,.4)', color: C, borderRadius: 6, cursor: 'pointer' }}>EDIT</button>
