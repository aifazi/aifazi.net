...
function Stat({ label, value, color = 'var(--text)', sub }) {
  return (
    <div style={{ background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 8, padding: '14px 16px' }}>
      ...
      {sub && <div style={{ fontFamily: MONO, fontSize: 11, color: 'var(--muted)', marginTop: 2 }}>{sub}</div>}
    </div>
  )
}
      ...
      {d.revenue_by_day?.length > 0 ? (
        <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 10, padding: 16, marginBottom: 16 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12, flexWrap: 'wrap', gap: 8 }}>
            <div style={{ fontFamily: MONO, fontSize: 11, letterSpacing: 2, color: 'var(--muted)' }}>REVENUE · LAST 30 DAYS</div>
            <span style={{ fontFamily: MONO, fontSize: 11, color: 'var(--muted)' }}>{d.revenue_by_day.length} days · {convOrders} orders today</span>
          </div>
            ...
            {d.revenue_by_day.map((x, i) => {
              ...
              return (
                <div key={x.date} title={`${fmt(x.date)}: ${money(x.revenue_cents)}`} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                  ...
                  {isLast && <span style={{ fontFamily: MONO, fontSize: 11, color: 'var(--muted)', transform: 'rotate(-40deg)' }}>{money(x.revenue_cents)}</span>}
                </div>
              )
            })}
          </div>
              ...
              {d.top_products.map((p, i) => {
                ...
                return (
                  <div key={p.name} style={{ background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 8, padding: '9px 12px' }}>
                    ...
                    <div style={{ fontFamily: MONO, fontSize: 11, color: 'var(--muted)', marginTop: 4 }}>{p.units} units</div>
                  </div>
                )
              })}
            </div>
