...
@router.get("/migrate")
async def run_migrate(_: dict = Depends(require_staff)):
    """Idempotent — create audit_logs table + indexes if they don't already exist."""
    return audit_migrate()


@router.get("/check")
async def check_table(_: dict = Depends(require_staff)):
...
@router.post("")
async def log_event(body: dict, _: dict = Depends(require_staff)):
    ok = audit_record(
