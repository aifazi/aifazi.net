...
def _user_owns_ticket(ticket: dict, user: dict | None) -> bool:
    ...
    if ticket_user_id and ticket_email:
        return ticket_user_id == user_id and ticket_email == user_email
    if ticket_user_id:
...
async def _auto_create_forum_account(name: str, email: str, bg: BackgroundTasks) -> str | None:
    ...
    try:
        res = supabase.table("users").insert({
            ...
            "email_verified": True, "role": "member",
        ...
        bg.add_task(send_email, email,
            subject or "Your aifazi.net account has been created",
            html or _new_account_email_html(username, password, login_url),
            f"aifazi.net login -- username: {username}  password: {password}  url: {login_url}",
...
async def submit_ticket(body: TicketBody, bg: BackgroundTasks, user: dict | None = Depends(_optional_user)):
    ...
    bg.add_task(send_email, body.email, conf_subject, conf_html, conf_text, "ticket_confirmation")
...
async def admin_add_message(
    ...
    bg.add_task(send_email, ticket.get("email", ""), bg_subject or fallback_subject, bg_html or fallback_html, bg_text, "ticket_reply")
