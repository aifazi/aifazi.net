"""routers/fivem.py  v5.1 — Bidirectional txAdmin sync, full history, real-time.

Fixes in v5.1:
  • pending-sync returns a LIST (not dict) — Lua expects an array
  • StatusUpdate accepts 'players' list from Lua heartbeat (was silently dropped)
  • txadmin-event handles 'whitelistPlayer' and 'playerWhitelisted' both ways
  • _sync_to_txadmin tries license: prefix variants before giving up
  • fivem_realtime_events cleanup wired to cron endpoint
  • history source labels cover all sync_source values
"""
...
def _email_approved(name: str, char: str, note: str | None) -> tuple[str, str]:
    subject = "🎮 Your AIFAZI RP Whitelist Application — APPROVED"
    html = f"""
<div style="font-family:sans-serif;max-width:560px;margin:0 auto;background:#0d1117;color:#e6edf3;border-radius:12px;overflow:hidden">
  ...
  <div style="padding:32px">
    <h1 style="color:#00FF88;font-size:24px;margin:0 0 8px">✅ Whitelist Approved!</h1>
    ...
    <div style="background:#161b22;border:1px solid #30363d;border-radius:8px;padding:16px;margin-bottom:24px">
      ...
      <div style="font-size:18px;font-weight:700;color:#00FF88">{char}</div>
      <div style="font-size:13px;color:#8b949e;margin-top:4px">Applied by {name}</div>
    </div>
    {f'<div style="background:#161b22;border-left:3px solid #00D4FF;padding:12px 16px;border-radius:4px;margin-bottom:24px"><div style="font-size:11px;color:#8b949e;margin-bottom:4px">NOTE FROM ADMIN</div><div style="color:#e6edf3">{note}</div></div>' if note else ''}
  ...
  <div style="background:#161b22;padding:16px;text-align:center;font-size:11px;color:#8b949e">
    AIFAZI RP · aifazi.net · This email was sent because you applied for whitelist.
  </div>
</div>"""
    return subject, html

def _email_denied(name: str, char: str, note: str | None) -> tuple[str, str]:
    subject = "Your AIFAZI RP Whitelist Application — Decision"
    html = f"""
<div style="font-family:sans-serif;max-width:560px;margin:0 auto;background:#0d1117;color:#e6edf3;border-radius:12px;overflow:hidden">
  <div style="background:linear-gradient(135deg,#ff4757,#ff6b81);padding:3px"></div>
  <div style="padding:32px">
    <h1 style="color:#ff4757;font-size:24px;margin:0 0 8px">❌ Application Not Approved</h1>
    <p style="color:#8b949e;margin:0 0 24px">Thank you for applying to AIFAZI RP, {name}.</p>
    <div style="background:#161b22;border:1px solid #30363d;border-radius:8px;padding:16px;margin-bottom:24px">
      <div style="font-size:12px;color:#8b949e;text-transform:uppercase;letter-spacing:1px;margin-bottom:4px">Character</div>
      <div style="font-size:18px;font-weight:700;color:#ff4757">{char}</div>
    </div>
    {f'<div style="background:#161b22;border-left:3px solid #ff4757;padding:12px 16px;border-radius:4px;margin-bottom:24px"><div style="font-size:11px;color:#8b949e;margin-bottom:4px">REASON</div><div style="color:#e6edf3">{note}</div></div>' if note else '<div style="background:#161b22;border-left:3px solid #ff4757;padding:12px 16px;border-radius:4px;margin-bottom:24px"><div style="color:#e6edf3">No specific reason provided.</div></div>'}
  ...
  <div style="background:#161b22;padding:16px;text-align:center;font-size:11px;color:#8b949e">
    AIFAZI RP · aifazi.net
  </div>
</div>"""
    return subject, html

def _email_reset(name: str, char: str) -> tuple[str, str]:
    subject = "Your AIFAZI RP Application — Reset to Pending"
    html = f"""
<div style="font-family:sans-serif;max-width:560px;margin:0 auto;background:#0d1117;color:#e6edf3;border-radius:12px;overflow:hidden">
  <div style="background:#facc15;padding:3px"></div>
  <div style="padding:32px">
    <h1 style="color:#facc15;font-size:24px;margin:0 0 8px">⏳ Application Reset</h1>
    <p style="color:#8b949e;margin:0 0 24px">Hi {name}, your application for <strong style="color:#facc15">{char}</strong> has been reset to pending for re-review. No action is needed — our team will review it shortly.</p>
  </div>
  <div style="background:#161b22;padding:16px;text-align:center;font-size:11px;color:#8b949e">AIFAZI RP · aifazi.net</div>
</div>"""
    return subject, html
...
<div style="font-family:sans-serif;max-width:560px;margin:0 auto;background:#0d1117;color:#e6edf3;border-radius:12px;overflow:hidden">
  ...
  <div style="padding:32px">
    ...
    <p style="color:#8b949e;margin:0 0 24px">Hi {name}, your whitelist application is now waiting for staff review.</p>
    <div style="background:#161b22;border:1px solid #30363d;border-radius:8px;padding:16px">
      <div style="font-size:12px;color:#8b949e;text-transform:uppercase;letter-spacing:1px;margin-bottom:4px">Character</div>
      <div style="font-size:18px;font-weight:700;color:#00D4FF">{char}</div>
    </div>
  </div>
  <div style="background:#161b22;padding:16px;text-align:center;font-size:11px;color:#8b949e">AIFAZI RP · aifazi.net</div>
</div>"""
...
<div style="font-family:sans-serif;max-width:560px;margin:0 auto;background:#0d1117;color:#e6edf3;border-radius:12px;overflow:hidden">
  ...
  <div style="padding:32px">
    ...
    <p style="color:#8b949e;margin:0 0 24px">Hi {name}, your queue priority for <strong style="color:#e6edf3">{char}</strong> has been updated.</p>
    <div style="background:#161b22;border:1px solid #30363d;border-radius:8px;padding:16px">
      <div style="font-size:12px;color:#8b949e;text-transform:uppercase;letter-spacing:1px;margin-bottom:4px">Priority</div>
      <div style="font-size:18px;font-weight:700;color:#facc15">{tier} · {level}</div>
      <div style="font-size:13px;color:#8b949e;margin-top:6px">Expires: {expiry}</div>
    </div>
  </div>
  <div style="background:#161b22;padding:16px;text-align:center;font-size:11px;color:#8b949e">AIFAZI RP · aifazi.net</div>
</div>"""
...
<div style="font-family:sans-serif;max-width:560px;margin:0 auto;background:#0d1117;color:#e6edf3;border-radius:12px;overflow:hidden">
  ...
  <div style="padding:32px">
    ...
    <p style="color:#8b949e;margin:0 0 24px">Hi {name}, a ban has been applied to your AIFAZI RP access.</p>
    <div style="background:#161b22;border-left:3px solid #ff4757;padding:12px 16px;border-radius:4px;margin-bottom:18px">
      ...
      <div style="color:#e6edf3">{char}</div>
    </div>
    <div style="background:#161b22;border-left:3px solid #ff4757;padding:12px 16px;border-radius:4px">
      ...
      <div style="color:#e6edf3">{reason or 'No reason provided.'}</div>
      <div style="font-size:12px;color:#8b949e;margin-top:10px">Expires: {expiry}</div>
    </div>
  </div>
  <div style="background:#161b22;padding:16px;text-align:center;font-size:11px;color:#8b949e">AIFAZI RP · aifazi.net</div>
</div>"""
...
<div style="font-family:sans-serif;max-width:560px;margin:0 auto;background:#0d1117;color:#e6edf3;border-radius:12px;overflow:hidden">
  ...
  <div style="padding:32px">
    ...
    <p style="color:#8b949e;margin:0">Hi {name}, the server ban for <strong style="color:#e6edf3">{char}</strong> has been lifted and is queued for server sync.</p>
  </div>
  <div style="background:#161b22;padding:16px;text-align:center;font-size:11px;color:#8b949e">AIFAZI RP · aifazi.net</div>
</div>"""
    return subject, html
...
async def _send_whitelist_email(app: dict, status: str, note: str | None = None, extra: Optional[dict] = None) -> None:
    ...
    if not to_email or "@" not in to_email:
        log.info("No email found for discord_id=%s — skipping notification", discord_id)
        return
...
# ─── Internal: push one approval to txAdmin + update DB row ──────────────────
async def _sync_to_txadmin(app: dict, approved_by: str, source: str) -> dict:
    ...
    for ident in candidates:
        ...
        if ok:
            ...
            log.warning("txAdmin identifier %s failed: %s — trying next", ident, msg)
            txadmin_note = msg

    if not candidates:
        log.warning("No identifier for app %s — skipping txAdmin push", app.get("id"))
...
async def search_whitelist(
    ...
    email column is optional — gracefully excluded if the column doesn't exist yet.
    """
    ...
    if q and q.strip():
        ...
        try:
            ...
            if "email" in str(e).lower():
                # email column not migrated yet — search without it
                log.warning("email column missing in fivem_whitelist — run migration. Searching without it.")
...
async def whitelist_history(limit: int = 100, _: dict = Depends(require_staff)):
    ...
    SOURCE_LABELS = {
        ...
        "website_manual":    ("✏️ Manual add (website)",          "#a78bfa"),
    ...
    for r in rows:
        ...
        label, color = SOURCE_LABELS.get(src, (f"📋 {src}", "#6b7280"))
...
async def review_whitelist(
    ...
    if body.status == current_status:
        ...
        raise HTTPException(409, f"Player is {status_labels.get(current_status, current_status)} — no change made.")
    ...
    if body.status == "approved":
        ...
        return {
            "message":  "Approved — syncing to server + sending email in background.",
            "app_id":   app_id,
            "syncing":  True,
        }
...
async def manual_add_whitelist(
    ...
    return {"message": "Player manually whitelisted — syncing to server in background.", "app": app}

# ─── Fallback Lua polling endpoints ──────────────────────────────────────────
@router.get("/whitelist/pending-sync-legacy")
async def pending_sync_legacy(request: Request):
    """
    FIX: was returning a dict — Lua expects a bare JSON array.
    ...
    for r in rows:
        ...
        r["license"] = lic
        out.append(r)
    # Return bare array — Lua does: local entries = json.decode(body)
    return out
...
# ─── txAdmin → Website  (Lua forwards txAdmin:events:* here) ────────────────
...
async def receive_txadmin_event(
    ...
    Handles:
      whitelistPreApproval  — admin approved/removed via txAdmin panel
      playerWhitelisted     — player passed whitelist check on join
      whitelistPlayer       — alias for playerWhitelisted (older txAdmin)
      whitelistRequest      — player was rejected and request was logged
    """
    _check_token(request)
    event = body.event
    data  = body.data
    ...
    if event == "whitelistPreApproval":
        ...
        identifier = data.get("identifier", "")   # "license:abc…"
        ...
        if action == "added" and identifier:
            ...
            if matched_app:
                ...
                supabase.table("fivem_whitelist").update(upd).eq("id", app["id"]).execute()
                log.info("txAdmin→website: approved %s by %s", identifier, admin_name)
                ...
                # No existing application — auto-create an approved entry
                ...
                log.info("txAdmin→website: auto-created approved entry for %s", identifier)
                background_tasks.add_task(_push_realtime, "whitelist_txadmin_approved", {
        ...
        # Player rejected — just log it for visibility
        log.info("txAdmin whitelist request: %s", data)

    # ── BAN SYNC: txAdmin → Website ─────────────────────────────────────────
    elif event == "playerBanned":
        # txAdmin banned a player — create/update the ban record on website
        identifier  = data.get("license") or data.get("identifier", "")
        ...
        if identifier:
            ...
            if not existing.data:
                ...
                log.info("txAdmin→website: ban synced for %s by %s", identifier, admin_name)
        ...
        # txAdmin unbanned a player — lift the ban on website
        identifier = data.get("license") or data.get("identifier", "")
        admin_name = data.get("adminName", "txAdmin")
        if identifier:
            ...
            log.info("txAdmin→website: unban synced for %s by %s", identifier, admin_name)
...
async def create_ban(
    ...
    return {"message": "Player banned — queued for server sync.", "ban": ban}
...
async def unban_player(
    ...
    return {"message": "Player unbanned — queued for server sync.", "ban": ban}
...
async def get_server_status():
    ...
    if status == "online":     msg = f"Online — {players}/{d.get('max_players',48)} players"
    elif status == "degraded": msg = f"Starting up… (last seen {_last_seen_str(age)})"
...
async def check_connect_session(body: ConnectSessionRequest, request: Request):
    ...
    for token_row in recent_tokens.data or []:
        ...
        supabase.table("fivem_connect_tokens").update({"used": True}).eq("token_id", token_row["token_id"]).execute()
...
async def verify_connect_token(body: VerifyTokenRequest, request: Request):
    ...
    supabase.table("fivem_connect_tokens").update({"used": True}).eq("token_id", payload["jti"]).execute()
    return {
        "approved": True,
