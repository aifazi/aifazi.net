...
async def collection_browse(
    ...
    try:
        ...
        docs = _normalize_list(res.data or [])
...
async def user_set_role(user_id: str, body: UserActionBody, _: dict = Depends(require_staff)):
    if not body.role:
...
async def user_send_reset(user_id: str, bg: BackgroundTasks, _: dict = Depends(require_staff)):
    ...
    bg.add_task(send_email, row["email"], subject, html, text, "password_reset_admin", row.get("username") or "")
...
async def user_send_email(user_id: str, body: UserActionBody, bg: BackgroundTasks, _: dict = Depends(require_staff)):
    ...
    bg.add_task(send_email, row["email"], subject, html, body.message, "admin_user_message", row.get("username") or "")
