...
async def discord_login(redirect: str = ""):
    ...
    # Append destination path to state separated by ':'
    # urlencode() below handles encoding of ':' and '/' — do NOT pre-quote 'redirect'
    # here (that would cause double-encoding: %2F → %252F).
    if redirect:
        state = f"{state}:{redirect}"
    # FIX: use urlencode() so every value is properly percent-encoded once.
    # Previously, manual f-string concatenation left redirect_uri and state
    # unencoded, causing Discord or intermediate proxies to misinterpret
    # the '/' in the dest path as a URL separator → state arrived double-encoded.
...
async def discord_callback(background_tasks: BackgroundTasks, code: str = "", error: str = "", state: str = ""):
    ...
    # state format: "{random_token}:{dest_path}" (dest_path may be empty)
    dest = "/profile"
    if state and ":" in state:
        dest = state.split(":", 1)[1] or "/profile"

    # FIX: URL-encode dest so '/' in paths like '/forum/profile' are safe in a query value.
    # This ensures the frontend receives dest=%2Fforum%2Fprofile (single-encoding)
    # rather than a raw slash that some browsers re-encode on the next navigation.
    safe_dest = _urlparse.quote(dest, safe="/")   # keep '/' readable but encode everything else
