...
def _verify_signature(request: Request, body: bytes) -> None:
    ...
    # BUG FIX #6: was silently accepting in dev mode — now only allow in
    # explicit local dev (ENV != production AND neither secret is set).
    if not IS_PRODUCTION and not WEBHOOK_SECRET and not FIVEM_SERVER_SECRET:
        log.warning("No webhook secrets configured — accepting unauthenticated call (local dev only)")
        return

    raise HTTPException(403, "Invalid or missing webhook signature. Set FIVEM_SERVER_SECRET in .env.")
